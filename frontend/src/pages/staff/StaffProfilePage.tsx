...
function Field({
  ...
  focused: boolean; onFocus: () => void; onBlur: () => void; right?: React.ReactNode
}) {
  return (
    <div>
...
export default function StaffProfilePage() {
  ...
  const [email, setEmail] = useState('staff@inventra.in')
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ ...card, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 0 }}>
            <div style={{ position: 'relative', marginBottom: 16 }}>
              ...
              <button style={{ position: 'absolute', bottom: 0, right: 0, width: 28, height: 28, borderRadius: '50%', background: '#724B68', border: '2px solid #fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }} title="Change photo">
                <Camera size={13} color="#fff" />
              </button>
            </div>
            <div style={{ fontWeight: 800, fontSize: 18, color: '#1F2933', fontFamily: 'Poppins, Inter, sans-serif', marginBottom: 4 }}>{name}</div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'rgba(37,99,235,0.08)', color: '#2563eb', padding: '4px 12px', borderRadius: 20, fontSize: 12, fontWeight: 700, marginBottom: 8 }}>
          ...
          <div style={card}>
            ...
            <form onSubmit={handleUpdatePassword} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              <Field label="Current Password" icon={Lock} type={showCur ? 'text' : 'password'} value={curPass} onChange={setCurPass}
                ...
                right={<button type="button" onClick={() => setShowCur(!showCur)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af', display: 'flex', padding: 0 }}>{showCur ? <EyeOff size={15} /> : <Eye size={15} />}</button>}
              />
              <Field label="New Password" icon={Lock} type={showNew ? 'text' : 'password'} value={newPass} onChange={setNewPass}
                ...
                right={<button type="button" onClick={() => setShowNew(!showNew)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af', display: 'flex', padding: 0 }}>{showNew ? <EyeOff size={15} /> : <Eye size={15} />}</button>}
              />
              <Field label="Confirm Password" icon={Lock} type={showConf ? 'text' : 'password'} value={confPass} onChange={setConfPass}
                ...
                right={<button type="button" onClick={() => setShowConf(!showConf)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af', display: 'flex', padding: 0 }}>{showConf ? <EyeOff size={15} /> : <Eye size={15} />}</button>}
              />
