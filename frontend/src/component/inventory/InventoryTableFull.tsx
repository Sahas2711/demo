...
import { Search, Pencil, Trash2, Filter, PackageMinus, PackagePlus } from 'lucide-react'
...
export default function InventoryTableFull({ products, onEdit, onDelete, onStockAdjust }: Props) {
  ...
  function handleStockSave(p: Product) {
    if (onStockAdjust && stockQty !== '') {
      onStockAdjust(p.id, Number(stockQty), p.reorderLevel)
    }
    setStockEditId(null)
    setStockQty('')
  }
    ...
    <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #E7E9ED', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '16px 24px', borderBottom: '1px solid #E7E9ED', display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ position: 'relative', minWidth: 220 }}>
          ...
          <input placeholder="Search product or HSN…" value={search} onChange={e => setSearch(e.target.value)}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
          <thead>
            <tr style={{ background: '#F5F6F8' }}>
              {['Product Name', 'HSN Code', 'Category', 'Price', 'GST %', 'Stock Qty', 'Reorder Lvl', 'Status', 'Actions'].map(h => (
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={9} style={{ padding: '40px', textAlign: 'center', color: '#9ca3af', fontSize: 14 }}>No products found.</td></tr>
            ) : filtered.map(p => (
              ...
              >
                <td style={{ padding: '13px 20px', fontWeight: 600, color: '#1F2933' }}>
                  {p.name}
                  {p.description && <div style={{ fontSize: 11, color: '#9ca3af', fontWeight: 400, marginTop: 2 }}>{p.description.slice(0, 60)}{p.description.length > 60 ? '…' : ''}</div>}
                </td>
                  ...
                  {stockEditId === p.id ? (
                    <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                      <input type="number" min={0} value={stockQty} onChange={e => setStockQty(e.target.value)}
                        ...
                        autoFocus
                      />
                      <button onClick={() => handleStockSave(p)} style={{ padding: '4px 8px', borderRadius: 6, border: 'none', background: '#059669', color: '#fff', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>✓</button>
                      <button onClick={() => { setStockEditId(null); setStockQty('') }} style={{ padding: '4px 8px', borderRadius: 6, border: '1px solid #E7E9ED', background: '#fff', fontSize: 11, fontWeight: 600, cursor: 'pointer', color: '#4B5563' }}>✗</button>
                    </div>
                  ) : (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontWeight: p.lowStock ? 700 : 400, color: p.lowStock ? '#dc2626' : '#1F2933' }}>{p.quantityAvailable}</span>
                      {onStockAdjust && (
                        <button onClick={() => { setStockEditId(p.id); setStockQty(String(p.quantityAvailable)) }}
                          title="Adjust stock"
                ...
                <td style={{ padding: '13px 20px' }}>
                  {confirmId === p.id ? (
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      ...
                      <button onClick={() => handleDelete(p.id)} style={{ padding: '4px 10px', borderRadius: 6, border: 'none', background: '#ef4444', color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>Yes</button>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={() => onEdit(p)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', borderRadius: 7, border: '1.5px solid #E7E9ED', background: '#fff', fontSize: 12, fontWeight: 600, color: '#724B68', cursor: 'pointer', transition: 'all 0.15s' }}
                      ...
                      <button onClick={() => setConfirmId(p.id)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', borderRadius: 7, border: '1.5px solid #E7E9ED', background: '#fff', fontSize: 12, fontWeight: 600, color: '#ef4444', cursor: 'pointer', transition: 'all 0.15s' }}
