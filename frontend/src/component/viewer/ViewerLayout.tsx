...
import {
  ...
  LogOut, Package2, ChevronLeft, ChevronRight, Menu, User, Calculator,
} from 'lucide-react'
...
function ViewerSidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  return (
    <aside style={{
      ...
      {/* Viewer badge */}
      {!collapsed && (
        <div style={{ margin: '12px 12px 4px', background: 'rgba(5,150,105,0.2)', borderRadius: 8, padding: '6px 12px', display: 'flex', alignItems: 'center', gap: 6 }}>
          <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#34d399' }} />
          <span style={{ fontSize: 11, color: '#6ee7b7', fontWeight: 700, letterSpacing: '0.5px', textTransform: 'uppercase' }}>Read Only</span>
        </div>
      )}
      ...
      <nav style={{ flex: 1, padding: '8px 8px', display: 'flex', flexDirection: 'column', gap: 2 }}>
        {NAV.map(({ icon: Icon, label, path }) => {
          const active = pathname === path || (path !== '/viewer' && pathname.startsWith(path))
          return (
...
function ViewerTopBar({ onMenuClick }: { onMenuClick: () => void }) {
  ...
  return (
    <header style={{
      ...
      <button onClick={onMenuClick} style={{ display: 'none', background: 'none', border: 'none', cursor: 'pointer', color: '#4B5563', padding: 4 }} className="viewer-menu-btn">
        <Menu size={22} />
      </button>
      ...
      <div style={{ marginLeft: 'auto', position: 'relative' }}>
        ...
        >
          <div style={{ width: 34, height: 34, borderRadius: '50%', background: '#2d5282', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 14 }}>
            {user?.name?.charAt(0)?.toUpperCase() || 'V'}
          </div>
          <div style={{ textAlign: 'left' }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#1F2933' }}>{user?.name || 'Viewer'}</div>
        ...
        {dropOpen && (
          <div style={{
            ...
            <Link to="/viewer/profile" onClick={() => setDropOpen(false)} style={{
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#F5F6F8', fontFamily: 'Poppins, Inter, sans-serif' }}>
      ...
      <style>{`
        @media (max-width: 768px) {
          .viewer-menu-btn { display: flex !important; }
        }
      `}</style>
    </div>
  )
